...
long long int pair_full (long long int number, long long int size, long long int (&a)[])
{
    for (long long int i = 0; i < size/2; ++i)
        for (long long int j = size; j > size/2; --j)
            if (a[i] + a[j] == number)
            {
//                cout << i+1 << ' ' << j+1 << endl;
                return 0;
...
long long int pair_linear (long long int number, long long int size, long long int (&a)[])
{
